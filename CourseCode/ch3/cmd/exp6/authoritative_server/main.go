...
func recvWorker(p *player) {
	for {
		...
		/*
			================ 【学生重点 第三章：客户端只上传输入】 ================
			客户端不直接修改位置和血量，只把 ACTION 发给服务器。
			服务器把最新输入放进 inputs[p.id]，真正的世界状态更新留给 update。
			这就是权威服务器模型中“客户端发意图，服务器给结果”的核心。
			================================================================
		*/
		mu.Lock()
		inputs[p.id] = msg
...
func update() string {
	/*
		================ 【学生重点 第三章：权威状态更新】 ================
		服务器是唯一真相持有者：移动、攻击、扣血都在这里计算。
		客户端即使本地显示画面，也不能绕过 update 自行决定胜负。
		每一帧计算完成后再 broadcast，让所有客户端看到同一个结果。
		============================================================
	*/
	frame++
	event := ""
