...
func runServer(addr string) error {
	printDivider("实验一 / Network Goroutine Server")
	logln("监听地址:", addr)
	logln("运行方式: 再打开 4 个终端，分别执行 4 次 client 模式。")
	logln("目标: 让服务器把每条连接的收包工作交给独立 goroutine，再观察主循环耗时。")
	...
	for frame := 1; frame <= 2; frame++ {
		if err := runConcurrentServerFrame(frame, positions, sessions); err != nil {
			return err
		}
	}
	...
	printDivider("Network Goroutine Server 结束")
	logln("提示: 对比 network_serial_server_demo，可以直接看到慢客户端还在，但主循环已经不再排队等它。")
	return nil
}
...
func runConcurrentServerFrame(frame int, positions map[int]int, sessions map[int]*ClientSession) error {
	printDivider(fmt.Sprintf("Frame %d / Network Goroutine Server", frame))
	...
	frameStart := time.Now()
	results := make(chan eventResult, len(sessions))
	...
	for _, session := range sortedSessions(sessions) {
		...
		go func(session *ClientSession) {
			defer wg.Done()
			logf("[并发收包] Frame %-2d 玩家%-2d 的收包 goroutine 已启动\n", frame, session.ID)
			...
			results <- eventResult{
				Event:      ev,
				ReceivedAt: time.Since(frameStart),
			}
		}(session)
	}
	dispatchCost := time.Since(frameStart)
	logf("[主循环] Frame %-2d 收包任务分发完成: %s (目标<100ms)\n", frame, formatMS(dispatchCost))
	...
	received := make(map[int]eventResult, len(sessions))
	for result := range results {
		if result.Err != nil {
			return result.Err
		}
		received[result.Event.PlayerID] = result
		logf("[收到] Frame %-2d 玩家%-2d  %-4s %+d  客户端延迟=%-8s 到达时间=%s\n",
			frame, result.Event.PlayerID, result.Event.Action, result.Event.DeltaX,
			formatMS(result.Event.Latency), formatMS(result.ReceivedAt))
	}

	totalCollectCost := time.Since(frameStart)
	logf("[摘要] Frame %-2d 所有消息收齐: %s\n", frame, formatMS(totalCollectCost))
	if dispatchCost > frameBudget {
		logln("[现象] 这不应该发生；如果看到这里超时，说明主循环本身也被拖住了。")
	} else {
		logln("[现象] 主循环很快完成了收包任务分发，没有阻塞在某一条慢连接上。")
	}
	if totalCollectCost > frameBudget {
		logln("[补充] 慢客户端仍然更晚到达，只是它被隔离在独立 goroutine 中，不再把主循环卡死。")
	}

	applyOrder := []int{1, 2, 3, 4}
	if frame == 2 {
		applyOrder = []int{4, 1, 2, 3}
	}
	for _, pid := range applyOrder {
		result := received[pid]
		positions[pid] = clamp(positions[pid]+result.Event.DeltaX, 0, trackWidth)
	}
...
func runClient(addr string, playerID int, latencyOverride time.Duration) error {
	printDivider("实验一 / Network Goroutine Client")
	logln("连接服务器:", addr)
	...
	printDivider("Network Goroutine Client 结束")
	logln("该玩家脚本执行完毕。")
	return nil
}
...
func usage() {
	fmt.Println("用法:")
	fmt.Println("  go run ./cmd/exp1/network_goroutine_server_demo server [-addr 127.0.0.1:9105]")
	fmt.Println("  go run ./cmd/exp1/network_goroutine_server_demo client -player 1 [-addr 127.0.0.1:9105] [-latency-ms 500]")
	fmt.Println("  go run ./cmd/exp1/network_goroutine_server_demo client 1 [-addr 127.0.0.1:9105] [-latency-ms 500]")
	fmt.Println()
