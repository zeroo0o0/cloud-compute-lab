...
func run(addr, player string, delay time.Duration) error {
	...
	for {
		...
		case "START_ACTION":
			/*
				================ 【学生辅助】制造 fast/slow 英雄输入差异 ================

				这里不是服务端核心逻辑，只是用 delay 模拟两个英雄的输入延迟：
				- fast：疾风英雄，20ms 后发出移动输入。
				- slow：断流英雄，500ms 后发出移动输入。

				如果你想理解“为什么 goroutine 能解耦”，主要看 04_network_goroutine_warzone/server。
				===============================================================
			*/
			time.Sleep(delay)
