...
func requirePythonTomllib() {
	bin := pythonBin()
	cmd := exec.Command(bin, "-c", "import tomllib; print('tomllib ok')")
	output, err := cmd.CombinedOutput()
	if err != nil {
		log.Fatalf("python env check failed: %s\n%s", err, strings.TrimSpace(string(output)))
	}
	log.Printf("python check ok (%s): %s", bin, strings.TrimSpace(string(output)))
}
