...
Server-B 的 `elapsed` 是目标端接收某个阶段数据的耗时；Server-A 的“玩家感知停机时间”是从冻结玩家开始，到收到 Server-B 完成确认为止的端到端时间。两者不必完全相等。

本实验在本机 `127.0.0.1` 上运行，耗时会受到 Go 调度、TCP 缓冲、系统负载影响，不需要追求严格倍数关系。课堂重点看趋势：
