...
//
// 预期行为：
//   Phase-1：收到数据库B的 VOTE-NO -> 协调者决定全局 ABORT -> 广播 GLOBAL-ABORT
//   Phase-2：所有参与者回滚，状态：INIT -> READY/ABORT -> ABORT
//
// 运行方式：go run ./fault_a
// =============================================================================
package main
...
func main() {
	...
	fmt.Println()
	fmt.Println("故障模型：参与者主动拒绝（一票否决）。")
	fmt.Println("介绍：某参与者拒绝提交，导致全局 ABORT。")

	state := newClusterState()
	workers := []*workerNode{
		{Name: "数据库A", Behavior: voteYes, ReqCh: make(chan voteRequest), DecisionCh: make(chan decisionMsg), Done: make(chan struct{})},
		{Name: "数据库B", Behavior: voteNo, ReqCh: make(chan voteRequest), DecisionCh: make(chan decisionMsg), Done: make(chan struct{})},
...
// runCoordinator 协调者：参与者拒票流程
//
//	INIT -> 发送投票 -> 存在NO -> 写入ABORT -> 广播GLOBAL-ABORT
...
// runWorker 参与者：根据 Behavior 决定投票行为
func runWorker(state *clusterState, w *workerNode) {
	for {
		select {
		case req := <-w.ReqCh:
			if w.Behavior == voteNo {
				state.setWorkerByName(w.Name, stateAbort, "VOTE-NO")
				req.Reply <- voteResponse{Worker: w.Name, Vote: voteNo}
			} else {
