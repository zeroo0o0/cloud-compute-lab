...
//
// 预期行为：
//   - Phase-1：协调者发送 VOTE-REQ，数据库B回复 VOTE-NO
//   - 协调者决定全局 ABORT，广播 GLOBAL-ABORT
...
//
// 教学要点：
//   - 2PC 的保守性，只要有一个参与者拒绝，整个事务回滚（一票否决）
//
// 运行方式：
//   go run ./fault_a
// =============================================================================
...
// runCoordinator 故障A：参与者拒票的协调者逻辑
//
// 流程：INIT -> 发送投票请求 -> 收集投票结果（含NO） -> 写入 ABORT -> 广播 GLOBAL-ABORT
...
func runWorker(state *clusterState, worker *workerNode) {
	for {
		...
		case req := <-worker.ReqCh:
			// 根据 Behavior 投票
			state.setWorkerByName(worker.Name, stateReady, "VOTE-"+string(worker.Behavior))
...
func main() {
	...
	fmt.Println()
	fmt.Println("某参与者拒绝提交，导致全局 ABORT。")
