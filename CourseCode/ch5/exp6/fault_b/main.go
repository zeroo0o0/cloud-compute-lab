...
//   Phase-1：协调者检测到部分参与者未响应
//   Phase-2：协调者写入 ABORT 决议，广播 GLOBAL-ABORT
//   所有已响应参与者回滚，状态：INIT -> READY -> ABORT
//
// 运行方式：go run ./fault_b
// =============================================================================
package main
...
func main() {
	...
	fmt.Println("介绍：某参与者长时间沉默，协调者等待超时后回滚。")
...
// runCoordinator 协调者：故障B流程
//
//	INIT -> 发送投票 -> 部分参与者超时 -> 写入ABORT -> 广播GLOBAL-ABORT
...
doneVotes:
	...
	if hasTimeout {
		// 检查哪些参与者未响应
		for i, w := range workers {
			if _, ok := votes[w.Name]; !ok {
				state.setWorker(i, stateReady, "超时无响应")
			}
		}
...
func runWorker(state *clusterState, w *workerNode) {
	for {
		select {
		case req := <-w.ReqCh:
			if w.Behavior == voteTimeout {
				// 模拟无响应：不回复投票，直接 continue 跳过
				continue
			}
