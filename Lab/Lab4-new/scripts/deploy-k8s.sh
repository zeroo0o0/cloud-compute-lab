...
REDIS_IMAGE="${4:-${REDIS_IMAGE:-redis:7-alpine}}"
...
retry 5 kubectl -n "$NAMESPACE" set image statefulset/lab4-redis redis="$REDIS_IMAGE"

echo "==> Wait for rollout"
retry 5 kubectl -n "$NAMESPACE" rollout status statefulset/lab4-redis --timeout=240s
