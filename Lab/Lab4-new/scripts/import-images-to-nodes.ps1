...
foreach ($node in $Nodes) {
  ...
  if ($Runtime -eq "containerd") {
    ssh @SshOptions $SshTarget "ctr -n k8s.io images import '$RemoteTar' && (crictl images 2>/dev/null | grep -E 'lab4|redis' || ctr -n k8s.io images ls | grep -E 'lab4|redis')"
  } elseif ($Runtime -eq "docker") {
    ssh @SshOptions $SshTarget "docker load -i '$RemoteTar' && docker images | grep -E 'lab4|redis'"
  } else {
    ssh @SshOptions $SshTarget "if command -v ctr >/dev/null 2>&1; then ctr -n k8s.io images import '$RemoteTar' && (crictl images 2>/dev/null | grep -E 'lab4|redis' || ctr -n k8s.io images ls | grep -E 'lab4|redis'); elif command -v docker >/dev/null 2>&1; then docker load -i '$RemoteTar' && docker images | grep -E 'lab4|redis'; else echo 'no containerd ctr or docker found' >&2; exit 1; fi"
  }
}
