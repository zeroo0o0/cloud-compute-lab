...
//   ROLE=gateway
//   NODE_MODE=remote
//   NODE_ADDRS=node-a=battleworld-node-0.battleworld-nodes:9311,...
//   MAP_ASSIGNMENTS=green=node-a:node-c,...
//   GATEWAY_ADDR=0.0.0.0:9310

func main() {
	dataRoot := resolveDataRoot()
	store, err := storage.NewStore(dataRoot)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[gateway] 初始化存储失败：%v\n", err)
		os.Exit(1)
	}

	cfg := cluster.LoadClusterConfig()
	cfg.NodeMode = "remote" // 网关模式下强制使用远程节点

	gameCluster, err := cluster.NewCluster(store, cfg)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[gateway] 初始化集群失败：%v\n", err)
		os.Exit(1)
	}
	if err := gameCluster.Start(); err != nil {
		fmt.Fprintf(os.Stderr, "[gateway] 启动集群失败：%v\n", err)
		os.Exit(1)
	}

	ln, err := net.Listen("tcp", protocol.GatewayAddr)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[gateway] 网关监听失败：%v\n", err)
		os.Exit(1)
	}
	defer ln.Close()

	fmt.Printf("[gateway] 网关已启动：%s\n", protocol.GatewayAddr)

	for {
		raw, err := ln.Accept()
		if err != nil {
			fmt.Fprintf(os.Stderr, "[gateway] 接入连接失败：%v\n", err)
			continue
		}
