...
type ClusterConfig struct {
	...
	MapAssignments map[string]MapAssignment
	NodeMode       string // "local" or "remote"
}
...
type Cluster struct {
	mu       sync.RWMutex
	store    *storage.Store
	nodes    map[string]NodeServiceInterface
...
func LoadClusterConfig() ClusterConfig {
	cfg := ClusterConfig{
		...
		MapAssignments: make(map[string]MapAssignment),
		NodeMode:       "local",
	}
	...
	// 解析 NODE_ADDRS: "node-a=127.0.0.1:9311,node-b=127.0.0.1:9312,node-c=127.0.0.1:9313"
	addrs := os.Getenv("NODE_ADDRS")
	if addrs == "" {
		addrs = "node-a=127.0.0.1:9311,node-b=127.0.0.1:9312,node-c=127.0.0.1:9313"
	}
	for _, pair := range strings.Split(addrs, ",") {
		parts := strings.SplitN(strings.TrimSpace(pair), "=", 2)
		if len(parts) == 2 {
			cfg.Nodes[parts[0]] = NodeConfig{ID: parts[0], Addr: parts[1]}
		}
	}

	// 解析 MAP_ASSIGNMENTS: "green=node-a:node-c,cave=node-b:node-c,ruins=node-a:node-b"
	...
	for _, pair := range strings.Split(assignments, ",") {
		...
		if len(parts) == 2 {
			...
			if len(nodes) == 2 {
				cfg.MapAssignments[mapID] = MapAssignment{Owner: nodes[0], Replica: nodes[1]}
			}
		}
	}
...
func NewCluster(store *storage.Store, cfg ClusterConfig) (*Cluster, error) {
	c := &Cluster{
		store:    store,
		nodes:    make(map[string]NodeServiceInterface),
	...
	for id, nc := range cfg.Nodes {
		if cfg.NodeMode == "remote" {
			c.nodes[id] = newRemoteNodeService(id, nc.Addr)
		} else {
			c.nodes[id] = newLocalNodeService(id, nc.Addr)
		}
	}

	// 地图-节点分配
	for mapID, placement := range cfg.MapAssignments {
		mapCfg := c.configs[mapID]
		c.owners[mapID] = placement.Owner
		c.replicas[mapID] = placement.Replica
		if node, ok := c.nodes[placement.Owner]; ok {
			node.InstallPrimaryMap(mapCfg)
			if cp, ok := store.LoadCheckpoint(mapID); ok {
				node.RestorePrimaryMap(mapCfg, *cp)
				if replica, ok := c.nodes[placement.Replica]; ok {
...
func (c *Cluster) SnapshotFor(username string) (*protocol.WorldState, error) {
	...
	nodes := make(map[string]NodeServiceInterface, len(c.nodes))
...
func (c *Cluster) sessionNode(username string) (*Session, NodeServiceInterface, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *Cluster) backgroundLoop() {
	...
	for {
		...
		case <-ticker.C:
			...
			sort.Strings(nodeIDs)
			nodes := make([]NodeServiceInterface, 0, len(nodeIDs))
			for _, nodeID := range nodeIDs {
