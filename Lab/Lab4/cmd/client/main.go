...
func main() {
	...
	for {
		...
		if transferOpen {
			targets := transferTargets(current)
			if len(targets) == 0 {
				addClientNote("当前没有可转移的在线玩家")
				setTransferOpen(false)
				drawUI()
				continue
			}
			if transferIndex >= len(targets) {
				transferIndex = 0
			}
			switch key {
			case 't', 'T', 27:
				...
				continue
			case 'w', 'W':
				transferIndex = (transferIndex - 1 + len(targets)) % len(targets)
				drawUI()
				continue
			case 's', 'S':
				transferIndex = (transferIndex + 1) % len(targets)
				drawUI()
				continue
			case 'a', 'A':
				if transferAmount > 1 {
			...
			case '1', '2', '3', '4', '5', '6', '7', '8', '9':
				transferAmount = int(key - '0')
				drawUI()
				continue
			case 'j', 'J', ' ':
				target := targets[transferIndex]
				msg = protocol.Message{Type: protocol.TypeTransfer, Target: target.Username, Amount: transferAmount}
				note = fmt.Sprintf("向 %s 发起 %d 份战利品转移（2PC）", target.Username, transferAmount)
			default:
				continue
			}
		} else if shopOpen {
...
func setTransferOpen(open bool) {
	uiMu.Lock()
	defer uiMu.Unlock()
	transferOpen = open
	if open {
...
func drawUI() {
	...
	sb.WriteString(renderBanner("烬原战境 Lab4", "即时战斗 / 中文彩色界面 / 多地图并行 / 多节点协同"))
	sb.WriteString("\n")
	for i := 0; i < height; i++ {
		left := ""
		if i < len(mapLines) {
	...
	if shopOpen {
		...
		sb.WriteString(colorGold + "转移模式：" + colorReset + colorDim + "W/S 选目标  A/D 调数量  J/空格 确认  T/Esc 关闭" + colorReset + "\n")
	} else {
		sb.WriteString(colorDim + "按键：W/A/S/D 移动  J 攻击  K 治疗  B 世界首领  P 商店  T 转移  1/2/3 切图  R 刷新  Q 退出" + colorReset + "\n")
	}
...
func buildSideLines(state *protocol.WorldState) []string {
	...
	for _, node := range state.Nodes {
		...
		leaderMark := ""
		if node.ID == state.Meta.LeaderID {
			leaderMark = colorGold + "★" + colorReset
		}
		lines = append(lines, fmt.Sprintf("%s %-8s %s gossip:%s 主:%d 备:%d",
			leaderMark, node.ID, status, paintMemberStatus(node.Status), len(node.PrimaryMaps), len(node.ReplicaMaps)))
	}
	lines = append(lines,
		panelTitle("Raft 元数据"),
		panelLine("Leader", valueOrText(state.Meta.LeaderID, "暂无")),
		panelLine("Term", fmt.Sprintf("%d", state.Meta.LeaderTerm)),
		panelLine("Log", fmt.Sprintf("%d", state.Meta.LogLength)),
		panelLine("Commit", fmt.Sprintf("%d", state.Meta.CommitIndex)),
		panelLine("健康节点", fmt.Sprintf("%d", state.Meta.HealthyNodes)),
		panelTitle("在线勇士"),
	)
	targets := transferTargets(state)
	if len(targets) == 0 {
		lines = append(lines, panelLine("转移目标", colorDim+"暂无其他在线玩家"+colorReset))
	} else {
		for i, target := range targets {
			prefix := " "
			if transferOpen && i == transferIndex {
				prefix = colorGold + ">" + colorReset
			}
			lines = append(lines, fmt.Sprintf("%s %-8s 地图:%-5s 节点:%s", prefix, target.Username, target.MapID, target.NodeID))
		}
		if transferOpen {
			lines = append(lines, panelLine("转移数量", colorGold+fmt.Sprintf("%d", transferAmount)+colorReset))
		} else {
			lines = append(lines, panelLine("入口", colorGold+"按 T 发起 2PC 转移"+colorReset))
		}
	}
	return lines
}
...
func transferTargets(state *protocol.WorldState) []protocol.OnlinePlayerView {
	targets := make([]protocol.OnlinePlayerView, 0, len(state.OnlinePlayers))
	for _, player := range state.OnlinePlayers {
		if player.Username == state.Self.Username {
			continue
		}
...
func paintMemberStatus(status string) string {
	switch status {
	case "alive":
		return colorGreen + "alive" + colorReset
	case "suspect":
		return colorOrange + "suspect" + colorReset
	case "dead":
