...
for node in "$@"; do
  ...
  case "$RUNTIME" in
    containerd)
      ssh "${SSH_OPTS[@]}" "$REMOTE_USER@$node" "ctr -n k8s.io images import '$REMOTE_TAR' && (crictl images 2>/dev/null | grep lab4 || ctr -n k8s.io images ls | grep lab4)"
      ;;
    docker)
      ssh "${SSH_OPTS[@]}" "$REMOTE_USER@$node" "docker load -i '$REMOTE_TAR' && docker images | grep lab4"
      ;;
    auto)
      ssh "${SSH_OPTS[@]}" "$REMOTE_USER@$node" "if command -v ctr >/dev/null 2>&1; then ctr -n k8s.io images import '$REMOTE_TAR' && (crictl images 2>/dev/null | grep lab4 || ctr -n k8s.io images ls | grep lab4); elif command -v docker >/dev/null 2>&1; then docker load -i '$REMOTE_TAR' && docker images | grep lab4; else echo 'no containerd ctr or docker found' >&2; exit 1; fi"
      ;;
    *)
