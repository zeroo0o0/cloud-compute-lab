...
本实验要求你把一个分布式文字游戏部署到自己的 Kubernetes 集群上，并让它具备 HPA 自动扩缩容、基础故障恢复和游戏状态一致性。
...
cmd/client              交互式客户端
cmd/admin               管理/状态查看工具
cmd/gateway-loadtest    压测客户端，scoretest 内部也会用类似逻辑
cloud/                  coordinator/map 的核心逻辑
cluster/                Kubernetes 生命周期、leader、pod deletion-cost 等支持代码
redisx/                 Redis 客户端封装
storage/                用户、session、checkpoint 存储
...
你可以用多阶段 Dockerfile 编译静态二进制，也可以用 Go 镜像直接运行；评分只看最终服务是否可用、是否可扩缩、是否能恢复。
...
如果要支持 Pod 回收保护，业务 Pod 还需要能访问 Kubernetes API。通常需要给业务 Pod 配置 ServiceAccount、Role 和 RoleBinding，让它们可以 patch 自己的 annotations。
...
如果本机没有配置 kubeconfig，脚本会自动退回 SSH 模式，提示输入主节点公网 IP、SSH 用户、端口和游戏入口地址。
...
评分脚本不按绝对 TPS 或公网延迟打分，避免把本机性能、集群硬件、公网质量算进成绩。高压项会先在当前集群里自校准；如果无法形成明确 HPA 压力，会显示 `WAIVED`，该项不计入总分。

注意：竞技评分会真实触发 HPA 扩容。重复评分前请先等几分钟，让 HPA 缩回稳定状态，否则“空闲不误扩”会受到上一轮压测残留副本影响。
...
```text
缩容或删除 Pod 时，readinessProbe 用于停止接收新流量，preStop 用于进入 drain 并保存状态，减少玩家感知。
```
