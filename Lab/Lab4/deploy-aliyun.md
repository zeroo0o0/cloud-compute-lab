...
# 推送 ACR
docker tag battleworld:latest registry.cn-shanghai.aliyuncs.com/cloud-lab/battleworld:latest
docker push registry.cn-shanghai.aliyuncs.com/cloud-lab/battleworld:latest
```
...
spec:
  containers:
  - name: battleworld
    image: registry.cn-shanghai.aliyuncs.com/cloud-lab/battleworld:latest
...
# 替换镜像地址
ACR="registry.cn-shanghai.aliyuncs.com/cloud-lab"
...
# 等待就绪
kubectl -n battleworld wait --for=condition=ready pod -l app=battleworld --timeout=120s

# 验证
kubectl -n battleworld get pods
...
kubectl -n battleworld delete deployment battleworld
...
# 等待所有 Pod 就绪
kubectl -n battleworld wait --for=condition=ready pod -l app=battleworld-gateway --timeout=120s
kubectl -n battleworld wait --for=condition=ready pod -l app=battleworld-node --timeout=120s
...
# 查看 StatefulSet DNS
kubectl -n battleworld get pods -o wide -l app=battleworld-node
...
```bash
# 进入 Gateway Pod
kubectl -n battleworld exec -it deployment/battleworld-gateway -- sh
...
```bash
# 查看所有资源
kubectl -n battleworld get all

# 查看 Pod 日志
kubectl -n battleworld logs -l app=battleworld-gateway -f
kubectl -n battleworld logs -l app=battleworld-node -f

# 进入 Pod 调试
kubectl -n battleworld exec -it deployment/battleworld-gateway -- sh
kubectl -n battleworld exec -it battleworld-node-0 -- sh

# 查看 PVC 状态
kubectl -n battleworld get pvc
kubectl -n battleworld describe pvc battleworld-data

# 查看 ConfigMap
kubectl -n battleworld get configmap
kubectl -n battleworld describe configmap battleworld-config

# 查看事件
kubectl -n battleworld get events --sort-by=.lastTimestamp
```
...
kubectl delete namespace battleworld
